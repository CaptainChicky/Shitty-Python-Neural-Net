...
class Training:
    ...
    #             1 = pure stochastic gradient descent (default, updates after each sample)
    #             >1 = mini-batch gradient descent (accumulates gradients over batch_size samples)
    #             len(training_data) = full batch gradient descent
    def __init__(self, neural_net, learning_rate, clip_value, cost_function=None, checkpoint_path=None, batch_size=1):
    ...
    def train(self, input_data, target_data, epochs, samples_per_epoch=None):
        ...
        Note:
            The batch_size is set in the Training constructor (self.batch_size).
            - batch_size=1: pure SGD
            - batch_size>1: mini-batch GD
            - batch_size=len(input_data): full batch GD
        """
        ...
        for epoch in range(epochs):
            ...
            if samples_per_epoch is not None and samples_per_epoch < len(input_data):
                ...
                # Need to shuffle sequential indices for mini-batch training
                # arrange returns a numpy array that can be shuffled in place
                sample_indices = np.arange(len(input_data))
                np.random.shuffle(sample_indices)

            # this sets up the batching, we cut the total number of indices into batch size
            num_samples = len(sample_indices)
            num_batches = int(np.ceil(num_samples / self.batch_size))

            for batch_idx in range(num_batches):
                batch_start = batch_idx * self.batch_size
                # bec we cant guarentee the total number of indices is divisible by the batch size, 
                # we have to ensure the last batch (possiibly smaller) is accounted for properly via below
                batch_end = min(batch_start + self.batch_size, num_samples)
                batch_indices = sample_indices[batch_start:batch_end]
                current_batch_size = len(batch_indices)

                # Initialize accumulated gradients (will be set on first iteration)
                ...
                for idx in batch_indices:
                    ...
                    if accumulated_gradients is None:
                        # First sample in batch so initialize accumulated gradients
                        accumulated_gradients = gradients
                    else:
                        # Add gradients from this sample to accumulated gradients
                ...
                for key in accumulated_gradients:
                    accumulated_gradients[key] /= current_batch_size

                # Update the parameters (weights and biases) using the averaged gradients
                self.update_parameters(accumulated_gradients, self.clip_value)
            ...
            # Checkpoint, so save model if this is the best cost so far
            ...
            batch_info = f" (batch_size={self.batch_size})" if self.batch_size > 1 else ""
            if samples_per_epoch is not None and samples_per_epoch < len(input_data):
                print(f"Epoch {epoch + 1}/{epochs}, Average Cost: {avg_cost} (trained on {num_samples_used}/{len(input_data)} samples){batch_info}{checkpoint_msg}")
            else:
                print(f"Epoch {epoch + 1}/{epochs}, Average Cost: {avg_cost}{batch_info}{checkpoint_msg}")
